...
// `powerset_by_id` and `ordering` are scaffolding for the in-progress
...
impl ASF {
    ...
    fn obtain_nidset(&mut self, nids: Vec<i32>) -> &Nidset {
        ...
        let nidset = self.nidset_by_id.entry(id).or_insert_with(|| Nidset { id, nids });
        &*nidset
    }
    ...
    pub fn new(recce: Recognizer) -> Result<Self> {
        ...
        loop {
            ...
            let _build_and_node_ids = and_node_ids
                .iter()
                .map(|id| ((bocage.and_node_predecessor(*id).unwrap_or(-1)), id))
                .collect::<Vec<_>>();
    ...
    fn compute_symches(&mut self, glade_id: usize) -> Result<&mut Glade> {
        ...
        let nidset = self
            .nidset_by_id
            .get(&glade_id)
            .unwrap_or_else(|| panic!("No glade found for glade ID {glade_id:?}"));
